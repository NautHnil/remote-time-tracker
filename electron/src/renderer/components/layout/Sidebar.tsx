            ...
            <div className="bg-gray-100 dark:bg-dark-800/30 rounded-lg p-2">
              ...
              {/* <div className="flex flex-wrap gap-1 mt-2">
                ...
                {permissions.isOrgOwner && !permissions.isSystemAdmin && (
                  <span className="px-1.5 py-0.5 text-[9px] font-semibold rounded-full bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30">
                    Org Owner
                  </span>
                )}
              </div> */}
...
const ThemeToggleButton: React.FC<ThemeToggleButtonProps> = ({
  ...
  return (
    ...
    >
      {isDark ? (
        <Icons.Sun className="w-4 h-4 text-yellow-500 flex-shrink-0" />
      ) : (
        <Icons.Moon className="w-4 h-4 text-indigo-500 flex-shrink-0" />
      )}
      {!isCollapsed && (
