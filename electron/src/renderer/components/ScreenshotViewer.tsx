...
const ScreenshotViewer: React.FC<ScreenshotViewerProps> = ({
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="bg-dark-800/50 backdrop-blur-sm border border-dark-700/50 rounded-xl px-4 py-3 flex items-center justify-between mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          {/* Source Toggle */}
          <div className="flex gap-0.5 bg-dark-800/80 rounded-lg p-0.5 border border-dark-700/50">
            <button
              onClick={() => {
                setViewSource("local");
                setPage(1);
          ...
          {/* Date filter */}
          <div className="flex items-center gap-2 bg-dark-800/80 border border-dark-700/50 rounded-lg px-3 py-1.5">
            <Icons.Calendar className="w-4 h-4 text-dark-400" />
            <input
              type="date"
              value={dateFilter}
              onChange={(e) => setDateFilter(e.target.value)}
              className="bg-transparent text-dark-200 text-sm focus:outline-none"
            />
            {dateFilter && (
              <button
                onClick={() => setDateFilter("")}
                className="text-dark-400 hover:text-white p-0.5 rounded hover:bg-dark-700/50"
                title="Clear filter"
              >
        ...
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="flex flex-col items-center gap-3">
              <div className="animate-spin rounded-full h-10 w-10 border-2 border-dark-700 border-t-primary-500"></div>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-dark-400">
            <div className="w-20 h-20 mb-4 rounded-full bg-dark-800 flex items-center justify-center">
              <Icons.Camera className="w-10 h-10 text-dark-600" />
            </div>
            ...
            <p className="text-sm mt-2 text-dark-500 max-w-md text-center">
              {viewSource === "local"
                ? "Local screenshots will appear here as you track time."
                : "Screenshots will appear here after syncing."}
            </p>
            {viewSource === "local" && (
              <p className="text-xs mt-3 text-dark-600 flex items-center gap-1.5">
          ...
          <>
            ...
            <div className="mt-6">
              <div className="flex items-center justify-center gap-2 text-dark-500 text-sm mb-4">
